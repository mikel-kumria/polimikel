...
class TemporalXORDataset(Dataset):
    def __init__(self, num_samples, v_th=2.0, seq_len=None, gap=0, noise_sigma=0.0):
        self.num_samples = num_samples
        self.v_th = v_th
        self.gap = gap
        self.noise_sigma = noise_sigma
        
        # Calculate required sequence length based on gap
        # 10 timesteps for A + gap + 10 timesteps for B + 10 timesteps for R
        required_length = 10 + gap + 10 + 10
        ...
        for _ in range(num_samples):
            ...
            idx_gap_end = idx_gap_start + self.gap
