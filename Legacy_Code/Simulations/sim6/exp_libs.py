...
class FakeQuantize(nn.Module):
    def __init__(self, bits=5, w_min=0.001, w_max=1.0, no_quantize=False):
        super().__init__()
        self.bits = bits
        self.levels = 2**bits
        ...
        if w_min >= w_max:
            raise ValueError(f"w_min ({w_min}) must be less than w_max ({w_max})")
        if bits <= 0:
    ...
    def forward(self, input):
        ...
        # If no_quantize is True, just return the input
        if self.no_quantize:
            return input
...
class QuantLinear(nn.Linear):
    def __init__(
        ...
        self.fake_quant = FakeQuantize(
            bits=bits, w_min=w_min, w_max=w_max, no_quantize=no_quantize
        )
    ...
    def extra_repr(self) -> str:
        return f"{super().extra_repr()}, bits={self.fake_quant.bits}, no_quantize={self.fake_quant.no_quantize}"
...
class SNNQUT_no_clochlea(nn.Module):
    def __init__(
        self,
        input_size,
        hidden_size,
...
class Lightning_SNNQUT_no_cochlea(pl.LightningModule):
    def __init__(
        self,
        input_size,
        hidden_size,
...
def get_tau_values(min_freq=50, max_freq=8000, num_neurons=16):
    ...
    frequencies = log_linspace(min_freq, max_freq, num_neurons)
    # Convert frequencies to time constants in milliseconds for low-pass filter
    tau_values = 1000 / (
        2 * np.pi * frequencies
