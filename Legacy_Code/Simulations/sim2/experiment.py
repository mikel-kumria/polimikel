...
def train_net(config, data_dir, num_epochs=10, concurrent_trials=8):
    ...
    output_reset_mechanism = "subtract"
    delta_t = 1 
...
def main():
    ...
    data_dir = os.path.join(current_dir, "../data/DATASET_FILTERBANK/TRAIN_VAL",)
    ...
    search_space = {
        ...
        "learning_rate": 1e-3,
        "hidden_threshold": tune.uniform(10,20),
    ...
    analysis = tune.run(
        ...
        name="MIKEL_LIF_quant_trial_3",
        storage_path=experiment_results,
        verbose=1,
        resume=True, # NOTE: resume is a Bool, let's see if "AUTO" works
    )
